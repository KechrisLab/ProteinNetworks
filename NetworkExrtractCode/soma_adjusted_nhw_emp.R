...
X1 <- Adjusted_AA
# set phenotype name
colnames(Y)[1] <- "EMP_NHW_Adjusted"
pheno <- "EMP_NHW_Adjusted"
...
# load global network
load('D:/Research/SOMA/EMP_NHW_Adjusted/5foldCVEMP_NHW_Adjusted_CCcoef_1_/Results/SmCCA5foldSamp100_0.5.Rdata')
# clustering
...
for (z in 1:length(mirGeneModule))
{
  ...
  for (i in 1:nrow(cor_result[[z]]))
  {
    ...
    if(size == 0){
      netD <- paste0("D:/Research/SOMA/EMP_NHW_Adjusted/5foldCVEMP_NHW_Adjusted_CCcoef_1_/", "NetworkEdgeCutpt0/")
      titlePre <- paste0("Net ")
      savePre <- paste0(netD, "Net")
    }else{
      netD <- paste0("D:/Research/SOMA/EMP_NHW_Adjusted/5foldCVEMP_NHW_Adjusted_CCcoef_1_/", "NetworkEdgeCutpt", size, "/")
    ...
    results <- plotMultiOmicsNetworks_PPR_Validation(Abar, bigCor2, mirGeneModule, 
                                                     ...
                                                     type ='EMP_NHW_Adjusted', 
                                                     id = pheno_data$sid,  P1 = p1,
                                                     folder = 'EMP_NHW_Adjusted',
  ...
  write.csv(pc_result,
            paste0('D:/Research/SOMA/EMP_NHW_Adjusted/5foldCVEMP_NHW_Adjusted_CCcoef_1_/pc_correlation_validation_', z, '.csv'),
            row.names = FALSE)
}
...
for (i in 10:200)
{
  ...
  load(paste0('D:/Research/SOMA/EMP_NHW_Adjusted/EMP_NHW_Adjustednet', temp_size, '_1AA.Rdata'))
  if (i == 10)
...
createNetworkFromIgraph(graph,"emp_nhw")

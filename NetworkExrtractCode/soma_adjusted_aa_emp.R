...
fev1_covariate_aa <- clinical %>%
  select(sid = sid,Platelets_P2,wbc_P2,BMI_P2, gender, smoking_status_P2,ccenter_P2, 
         Age_P2, pctEmph_Thirona_P2) %>% # include all relevant clinical covariates, phenotypes
  merge(x = ., y = X1, by = "sid", all.y = TRUE) %>% # merge with protein data
  drop_na()%>% # drop na value
  filter(aa == 1) # include only African American
...
#################################################################### Run SmCCNet
# define phenotype to EMP at P2
Y <- as.matrix(fev1_covariate_aa$pctEmph_Thirona_P2)
# define protein data
X1 <- Adjusted_AA
# set phenotype name
colnames(Y)[1] <- "EMP_AA_Adjusted"
pheno <- "EMP_AA_Adjusted"
...
############################## Obtain subnetworks of different size from the global network
# load global network 
load('D:/Research/SOMA/EMP_AA_Adjusted/5foldCVEMP_AA_Adjusted_CCcoef_1_/Results/SmCCA5foldSamp100_0.5.Rdata')
# clustering 
...
for (z in 1:length(mirGeneModule))
{
  ...
  for (i in 1:nrow(cor_result[[z]]))
  {
    ...
    if(size == 0){
      netD <- paste0("D:/Research/SOMA/EMP_AA_Adjusted/5foldCVEMP_AA_Adjusted_CCcoef_1_/", "NetworkEdgeCutpt0/")
      titlePre <- paste0("Net ")
      savePre <- paste0(netD, "Net")
    }else{
      netD <- paste0("D:/Research/SOMA/EMP_AA_Adjusted/5foldCVEMP_AA_Adjusted_CCcoef_1_/", "NetworkEdgeCutpt", size, "/")
    ...
    results <- plotMultiOmicsNetworks_PPR_Validation(Abar, bigCor2, mirGeneModule, 
                                                     ...
                                                     type ='EMP_AA_Adjusted', 
                                                     id = pheno_data$sid,  P1 = p1,
                                                     folder = 'EMP_AA_Adjusted',
  ...
  write.csv(pc_result,
            paste0('D:/Research/SOMA/EMP_AA_Adjusted/5foldCVEMP_AA_Adjusted_CCcoef_1_/pc_correlation_validation_', z, '.csv'),
            row.names = FALSE)
}
...
for (i in 10:199)
{
  ...
  load(paste0('D:/Research/SOMA/EMP_AA_Adjusted/EMP_AA_Adjustednet', temp_size, '_1AA.Rdata'))
  if (i == 10)
...
createNetworkFromIgraph(graph,"emp_aa")
